...
bayesmooth = function(x,sigma=NULL,v.est=FALSE,return.est=TRUE,basis="haar",prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=0,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
  ...
  if(is.null(sigma)){
    ...
    if(basis=="haar"){
      ...
      for(j in 0:(J-1)){
        index=(y[j+2,]!=0)|(vtable[j+2,]!=0)
        zdat.ash=fast.ash(y[j+2,index],sqrt(vtable[j+2,index]),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        wmean[j+1,index] = zdat.ash$PosteriorMean/2
        wmean[j+1,!index] = 0
      }
      wwmean=-wmean
  ...
  if(basis=="haar"){
    ...
    for(j in 0:(J-1)){
      ...
      zdat.ash=fast.ash(y[j+2,index],sqrt(vtable[j+2,index]),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
      wmean[j+1,index] = zdat.ash$PosteriorMean/2
      wvar[j+1,index] = zdat.ash$PosteriorSD^2/4
      wmean[j+1,!index] = 0
      wvar[j+1,!index] = 0
    }
    wwmean=-wmean
