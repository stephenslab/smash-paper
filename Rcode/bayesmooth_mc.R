...
bayesmooth.mc = function(x,sigma=NULL,v.est=FALSE,return.est=TRUE,basis="haar",prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=0,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
  ...
  if(is.null(sigma)){
    ...
    if(basis=="haar"){
      ...
      for(j in 0:(J-1)){
        ...
        zdat.ash=fast.ash(y[j+2,],sqrt(fac[j+1]*vtable[j+2,]),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
