...
require(inline)
library(Matrix)
source("~/ashwave/Rcode/jash_old.r")
...
bayesmooth = function(x,sigma=NULL,v.est=FALSE,v.basis=FALSE,return.est=TRUE,filter.number=1,family="DaubExPhase",prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=0,mixsd=NULL,VB=FALSE,jash=FALSE,weight=0.5){
  ...
  if(is.null(sigma)){
    ...
    if(basis=="haar"){
      ...
      for(j in 0:(J-1)){
        ...
        x.w.j=accessD(x.w,j)
        x.w.v.j=x.w.v[index]
        zdat.ash=ash(x.w.j,sqrt(x.w.v.j),prior=prior,multiseqoutput=TRUE,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
    ...
    if(basis=="haar"|v.basis==FALSE){
      ...
      for(j in 0:(J-1)){
        ...
        if(jash==TRUE){
          ...
          zdat.ash=ash(vdtable[j+2,],sqrt((vtable[j+2,]/virtable)),prior=prior,multiseqoutput=TRUE,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        }
        wmean[j+1,] = zdat.ash$PosteriorMean/2
      }
      wwmean=-wmean
  ...
  if(basis=="haar"){
    ...
    for(j in 0:(J-1)){
      zdat.ash=ash(y[j+2,],sqrt(vtable[j+2,]),prior=prior,multiseqoutput=TRUE,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=0)
      wmean[j+1,]=zdat.ash$PosteriorMean/2
    ...
    for(j in 0:(J-1)){
      ...
      x.w.j=accessD(x.w,j)
      x.w.v.j=x.w.v[index]
      zdat.ash=ash(x.w.j,sqrt(x.w.v.j),prior=prior,multiseqoutput=TRUE,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=0)
      x.pm = zdat.ash$PosteriorMean
      x.w = putD(x.w,j,x.pm)
    }
    mu.est=AvBasis(convert(x.w))
