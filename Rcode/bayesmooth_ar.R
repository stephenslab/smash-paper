...
s.est=function(x) {
  n=length(x)
  if(n<3){
    sqrt(1/(2)*sum((x[1:(n-1)]-x[2:n])^2))
  }else{
    sqrt(2/(3)*sum((1/2*x[1:(n-2)]-x[2:(n-1)]+1/2*x[3:n])^2))
  }
}
...
bayesmooth.ar = function(x,sigma=NULL,v.est=FALSE,return.est=TRUE,basis="haar",prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=0,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
  ...
  if(basis=="haar"){
    x.w.d = wd(x, filter.number=1, family="DaubExPhase", type = "station")
  }else{
    x.w.d = wd(x, filter.number=8, family="DaubLeAsymm", type = "station")
  }
  wmean = matrix(0,J,n) 
  wvar = matrix(0,J,n) 
  ...
  if(is.null(sigma)){
    ...
    if(basis=="haar"){
      x.w=x.w.d
      x.w.v1 = wd.var(var.est1.ini, 1, "DaubExPhase", type = "station")
      x.w.v2 = wd.var(var.est2.ini, 1, "DaubExPhase", type = "station")
      for(j in 5:(J-1)){
        y=accessD(x.w,j)
        win.size <- round(n/5)
        odd.boo <- (win.size%%2==1)
        win.size <- win.size+(1-odd.boo)
        sigma <- runmad(y,win.size,endrule="func")
        #tvar=madmad(y)*n
        tvar=sum(sigma^2)
        evar=accessC(x.w.v,j)
        if(tvar<sum(evar)){
          evar=evar
        }else{
          evar=evar*tvar/sum(evar)
        }
        zdat.ash=fast.ash(y,sqrt(evar),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        #zdat.ash=fast.ash(y,sigma,prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
    ...
    xv.w=wd(var.est, filter.number=1, family="DaubExPhase", type = "station")
    xv.w.v = wd.var(var.var.est, 1, "DaubExPhase", type = "station")
    ...
    #  zdat.ash=fast.ash(vdtable[j+2,],sqrt((fac[j+1]*vtable[j+2,])),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
    ...
    #var.est=cxxreverse.gwave(sum((var.est1.ini+var.est2.ini)/2),wmean,wwmean)
      for(j in 5:(J-1)){
        ...
        zdat.ash=fast.ash(y,sqrt(evar),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        #zdat.ash=fast.ash(y,sigma,prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        xv.pm = zdat.ash$PosteriorMean
        xv.w = putD(xv.w,j,xv.pm)
      }
    #xv.w=putC(xv.w,0,rep(sum((var.est1.ini+var.est2.ini)/2)/sqrt(n),n))
    var.est=AvBasis(convert(xv.w))
    var.est[var.est<=0]=1e-6
    sigma=sqrt(var.est)
  }
  if(basis=="haar"){
      x.w=x.w.d
      x.w.v = wd.var(sigma^2, 1, "DaubExPhase", type = "station")
      for(j in 5:(J-1)){
        y=accessD(x.w,j)
        win.size <- round(n/5)
        odd.boo <- (win.size%%2==1)
