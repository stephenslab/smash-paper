...
source("D:/Grad School/Spring 2013/multiscale_ash/ash/deltamethod.R")
library(wavethresh)
require(ashr)
require(Rcpp)
require(inline)
...
# in Donoho and Coifman's TI-denoising framework.
  dmat = matrix(0, nrow=J+1, ncol=n)
  ddmat = matrix(0, nrow=J+1, ncol=n)
  dddmat = matrix(0, nrow=J+1, ncol=n)
  dmat[1,] = var/mu^2
  ddmat[1,] = mu
  dddmat[1,] = var
  ...
  for(D in 0:(J-1)){
    ...
    for(l in 0:(2^D-1)){
      ...
      x = ddmat[D+1,ind]
      xv = dddmat[D+1,ind]
      lvar = xv[seq(from=1,to=nD-1, by=2)]/x[seq(from=1,to=nD-1, by=2)]^2+xv[seq(from=2,to=nD,by=2)]/x[seq(from=2,to=nD,by=2)]^2
      lsumx = x[seq(from=1,to=nD-1, by=2)] + x[seq(from=2,to=nD,by=2)]
      lsumxv = xv[seq(from=1,to=nD-1, by=2)] + xv[seq(from=2,to=nD,by=2)]

      rx = rshift(x)
      rxv = rshift(xv)
      rvar = rxv[seq(from=1,to=nD-1, by=2)]/rx[seq(from=1,to=nD-1, by=2)]^2+rxv[seq(from=2,to=nD,by=2)]/rx[seq(from=2,to=nD,by=2)]^2
      rsumx = rx[seq(from=1,to=nD-1, by=2)] + rx[seq(from=2,to=nD,by=2)]
      rsumxv = rxv[seq(from=1,to=nD-1, by=2)] + rxv[seq(from=2,to=nD,by=2)]
      dmat[D+2,ind] = c(lvar,rvar)
      ddmat[D+2,ind] = c(lsumx,rsumx)
      dddmat[D+2,ind] = c(lsumxv,rsumxv)
    }
  }
...
bayesmooth.var = function(x,sigma=NULL,v.est=FALSE,basis="haar"){
  n = length(x)
  J = log2(n)
  ...
  x.w = wd(x, filter.number=8, family="DaubLeAsymm", type = "station")
  ...
  if(is.null(sigma)){
    ...
    if(basis=="haar"){
      ...
      for(j in 0:(J-1)){
        ...
        zdat.ash=ash(y[j+2,],sqrt(fac[j+1]*vtable[j+2,]), usePointMass = FALSE, localfdr = FALSE, prior="nullbiased")
      ...
      x.w.v = wd.var(var.est.ini, type = "station")
      for(j in 0:(J-1)){
        zdat.ash=ash(accessD(x.w,j),sqrt(accessC(x.w.v,j)), usePointMass = FALSE, localfdr = FALSE, prior="nullbiased")
    ...
    vdtable=ltitable(var.est)$difftable
    vtable=varltitable(var.est,var.var.est)
    fac=c(2,1.5,1.3,1.15,1.1,1.07,rep(1,J-6))
    for(j in 0:(J-1)){
      zdat.ash=ash(vdtable[j+2,],sqrt((2*fac[j+1]*vtable[j+2,])), usePointMass = FALSE, localfdr = FALSE, prior="nullbiased")
      #zdat.ash=ash(vdtable[j+2,],sqrt((fac[j+1]*vtable[j+2,]/sqrt(vdtable[j+2,]))), usePointMass = FALSE, localfdr = FALSE, prior="nullbiased")
      alpha.mv=list(mean=zdat.ash$PosteriorMean,var=zdat.ash$PosteriorSD^2)      
      lp.mv = ff.moments_exp(alpha.mv$mean, alpha.mv$var)  #find mean and meansq of p
      wmean[j+1,] = lp.mv$mean     
      lq.mv = ff.moments_exp(-alpha.mv$mean, alpha.mv$var)  #find mean and meansq of q  
      wwmean[j+1,] = lq.mv$mean     

    }
    #for(j in (J-3):(J-1)){
    #  wmean[j+1,] = vdtable[j+2,]/2
    #}
    var.est=exp(reverse.pwave(log(sum((var.est1.ini+var.est2.ini)/2)),log(wmean),log(wwmean)))
  ...
  if(basis=="haar"){
    ...
    for(j in 0:(J-1)){
      fac=c(1.3,1.17,1.08,1.04,rep(1,J-4))
      zdat.ash=ash(y[j+2,],sqrt(vtable[j+2,]), usePointMass = FALSE, localfdr = FALSE, prior="nullbiased")
      wmean[j+1,] = zdat.ash$PosteriorMean/2
    }
    wwmean=-wmean
    mu.est=cxxreverse.gwave(tsum,wmean,wwmean)
