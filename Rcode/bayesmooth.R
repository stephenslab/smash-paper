...
#returns the non-decimated wavelet transform matrix for a given basis
...
mu.smooth=function(wc,data.var,basis,tsum,W2,post.var,prior,pointmass,nullcheck,VB,mixsd,gridmult,J,n){
  wmean = matrix(0,J,n) 
  wvar = matrix(0,J,n) 
  if(basis=="haar"){
    y=wc
    ...
    x.w=wc
    x.w.v=apply((rep(1,n*J)%o%data.var)*W2,1,sum)  #diagonal of W*V*W'   
    x.pm=rep(0,n)  
    for(j in 0:(J-1)){
...
#sigma - choice to provide the true variance function, should it be available
#v.est - boo, indicating if variance estimation should be performed in addition to mean estimation
#v.basis - bool, indicating if the same wavelet basis should be used for variance estimation as mean estimation. If false, defaults to Haar basis
#post.var - bool, indicating if the posterior mean should be returned
#filter.number, family - wavelet basis to be used, as in wavethresh
...
>>>>>>> 5959224d314492bb21d33a0964199aef915b9ba1
