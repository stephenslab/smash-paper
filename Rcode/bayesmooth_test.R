...
bayesmooth.test = function(x,sigma=NULL,v.est=FALSE,return.est=TRUE,basis="haar",prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=0,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
  ...
  if(is.null(sigma)){
    ...
    if(basis=="haar"){
      ...
      x.mod=x/sqrt((var.est1.ini+var.est2.ini)/2)
      y.mod=cxxtitable(x.mod)$difftable
      #y.mod=y/sqrt(vtable)
      fac=c(1.3,1.17,1.08,1.04,rep(1,J-4))
      for(j in 0:(J-1)){
        #zdat.ash=fast.ash(y.mod[j+2,],1,prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        zdat.ash=fast.ash(y.mod[j+2,],sqrt(2^(j+1)),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        #wmean[j+1,] = zdat.ash$PosteriorMean*sqrt(vtable[j+2,])/2
        wmean[j+1,] = zdat.ash$PosteriorMean/2
      }
      wwmean=-wmean
      mu.est=cxxreverse.gwave(sum(x.mod),wmean,wwmean)*sqrt((var.est1.ini+var.est2.ini)/2)
  ...
  if(basis=="haar"){
    ...
    wwvar=wvar
    #mu.est=cxxreverse.gwave(tsum,wmean,wwmean)
    mu.est=cxxreverse.gwave(sum(x.mod),wmean,wwmean)*sqrt(sigma^2)
