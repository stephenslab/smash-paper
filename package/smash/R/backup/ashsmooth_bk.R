  ...
  if(basis[[1]]=="haar"|v.basis==FALSE){
    ...
    for(j in 0:(J-1)){
      ...
      if((sum(is.na(wmean[j+1,]))>0)&(SGD==TRUE)){
        zdat.ash=shrink.wc(vdtable[j+2,ind.nnull],sqrt(vtable[j+2,ind.nnull]),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,mixcompdist=mixcompdist,gridmult=gridmult,jash=jash,df=min(50,2^(j+1)),SGD=FALSE)
        wmean[j+1,ind.nnull] = zdat.ash$PosteriorMean/2
        wmean[j+1,!ind.nnull] = 0
      }
      if(post.var==TRUE){
    ...
    for(j in 0:(J-1)){
      ...
      if((sum(is.na(x.pm))>0)&(SGD==TRUE)){
        zdat.ash=shrink.wc(x.w.j[ind.nnull],sqrt(x.w.v.j[ind.nnull]),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,mixcompdist=mixcompdist,gridmult=gridmult,jash=jash,df=min(50,2^(j+1)),SGD=FALSE)
        x.pm[ind.nnull] = zdat.ash$PosteriorMean
        x.pm[!ind.nnull] = 0
      }
      x.w = putD(x.w,j,x.pm)
...
threshold.haar <- function (vdtable, vtable, sigma.est, levels, type = "hard", policy = "universal")
{
    wmean <- vdtable[-1,]/2
    d <- NULL
    n <- dim(vdtable)[2]
...
threshold.var <- function (x.w, x.w.v, sigma.est, levels, type = "hard")
{
    d <- NULL
    n <- 2^nlevelsWT(x.w)
    J <- nlevelsWT(x.w)
...
ti.thresh=function(x,sigma=NULL,method="smash",filter.number=1,family="DaubExPhase",min.level=3) 
{
    ...
    if(filter.number==1&family=="DaubExPhase"){
      ...
      wmean=threshold.haar(vdtable,vtable,sigma.est,levels=0:(J-1-min.level),type="hard")
      wwmean=-wmean
      mu.est=cxxreverse_gwave(tsum,wmean,wwmean)
    }else{
      x.w=wd(x,filter.number=filter.number,family=family,type = "station")
      ...
      x.w.t=threshold.var(x.w,x.w.v,sigma.est,levels=(min.level):(J-1),type="hard")
