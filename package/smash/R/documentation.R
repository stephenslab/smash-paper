...
#' In the Gaussian case, the variances are allowed vary, but are similarly "spatially structured" as with the mean function. 
#' @details The functions \code{ashsmooth.gaus} and \code{ashsmooth.pois} perform smoothing for Gaussian and Poisson data respectively. The only required input is a vector of length 2^J for some integer J.
