function w = postblockmed(method,wcoef,l,sigma,thet0)

% postblockmed: Group the empirical wavelet coefficients into disjoint blocks 
%               of length l, and extract the wavelet coefficients needed by 
%               the RECBLOCKMED procedure.
% Usage
%               w = postblockmed(method,wcoef,l,sigma,thet0)
% Inputs
...
for i=1:nnew/l 
 ...
 threshval(:,i)=sign(dtilda).*max(0,abs(dtilda)-sigma*sqrt(diag(A))*...
     norminv(x));
end
