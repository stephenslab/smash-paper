function f = rechybblockmean(method,signal,l,thet0s,thet0b,h)

% rechybblockmean: Smoothing using a hybrid Bayesian block shrinkage scheme.  
%                  The POSTSINGLEMEAN procedure is applied on the first few 
%                  resolution levels (which is a user-choice) and the POSTBLOCKMEAN 
%                  procedure is applied on the remaining resolution levels.
% Usage
%                  f = rechybblockmean(method,signal,l,thet0s,thet0b,h)
% Inputs
%   method	      'Augment' for augmenting the data, 'Truncate' for truncating the 
%                  data
%   signal	       1-d Noisy signal, length(signal)= 2^J
%   l		          Length of the blocks. Level specific block length is enabled via 
%                  the choice l=[]
%   thet0s	       Parameter vector : [PIE0s,TAU0s] where PIE0s and TAU0s are the 
%                  initial values for the EM algorithm
%   thet0b	       Parameter vector : [PIE0b,TAU0b,RH0b] where PIE0b, TAU0b and 
%                  RH0b are the initial values for the maximization algorithm
%   h 		       Quadrature mirror filter for wavelet transform.
%		             Optional, default = Symmlet 8
% Outputs
%   f	         	 Estimate, obtained by applying nonlinear shrinkage on the 
...
for j=lev:J1
   wthresh(2^j+1:2^(j+1))=postsinglemean(wcoef(2^j+1:2^(j+1)),sigma,thet0s);
end
for j=(J1+1):J-1
 %allow for level specific block length
 if isempty(l), l2=j; else, l2=l; end
 wthresh(2^j+1:2^(j+1))=postblockmean(method,wcoef(2^j+1:2^(j+1)),l2,sigma,thet0b);
end  
