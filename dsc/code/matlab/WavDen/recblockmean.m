function f = recblockmean(method,signal,l,thet0,h)

% recblockmean: Smoothing using a Bayesian block shrinkage method.
%               It calls the POSTBLOCKMEAN procedure.
% Usage
%               f = recblockmean(method,signal,l,thet0,h)
% Inputs
...
% Outputs
%   f		       Estimate, obtained by applying nonlinear shrinkage on the wavelet 
%               coefficients.
% References
...
for j=lev:J-1
 ...
 wthresh(2^j+1:2^(j+1))=postblockmean(method,wcoef(2^j+1:2^(j+1)),l2,sigma,thet0);
end  
