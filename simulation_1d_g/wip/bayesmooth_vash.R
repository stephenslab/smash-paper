...
bayesmooth.vash = function(x,mu.t,prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=0,mixsd=NULL,VB=FALSE,weight=0.5){
  n = length(x)
  J = log2(n)
  ...
  wmean = matrix(0,J,n) 
    ...
    for(j in 0:(J-1)){
      vt=vash(vtable[j+2,],2)$PosteriorMean
      #vt[vt>vtable[j+2,]]=(vtable[j+2,])[vt>vtable[j+2,]]
      zdat.ash=fast.ash(vdtable[j+2,],sqrt((vt)),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
