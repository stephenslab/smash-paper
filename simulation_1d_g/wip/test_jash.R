...
source(file.path("D:/Grad School/Spring 2013/multiscale_ash/ash/bayesmooth_jash.R"))

system.time(var.est<-bayesmooth(X.s,v.est=TRUE))
system.time(var.est.j<-bayesmooth.jash(X.s,v.est=TRUE))
system.time(var.est.t<-bayesmooth.var(X.s,mu.t))


mse(var.est,sigma.t^2)
mse(var.est.j,sigma.t^2)
mse(var.est.t,sigma.t^2)

plot(sigma.t^2,type='l')
lines(var.est.j,col=2)
lines(var.est,col=4)