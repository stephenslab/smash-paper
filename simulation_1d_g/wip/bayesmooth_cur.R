...
bayesmooth.cur = function(x,mu.t,sigma.t,prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=0,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
  ...
  wmean = matrix(0,J,n) 
    ...
    wwmean=-wmean
    var.est=cxxreverse.gwave(sum(sigma.t^2),wmean,wwmean)
    return(var.est)
