...
bayestest.s8 = function(x,sigma=NULL,prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=2,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
 ...
 if(is.null(sigma)){
    ...
    x.w=x.w.d
    x.w.v = wd.var(sigma^2, filter.number=8, family="DaubLeAsymm", type = "station")
    for(j in 0:(J-1)){
...
bayestest.haar = function(x,sigma=NULL,prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=2,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
  if(!isTRUE(all.equal(J,trunc(J)))){stop("Error: number of columns of x must be power of 2")}
  if(length(sigma)==1){sigma=rep(sigma,n)}
 ...
 if(is.null(sigma)){
    ...
    var.est2.ini=(rshift(x)-x)^2/2
    x.w=x.w.d
    x.w.v1 = wd.var(var.est1.ini, filter.number=1, family="DaubExPhase", type = "station")
    for(j in 0:(J-1)){
      zdat.ash=fast.ash(accessD(x.w,j),sqrt(accessC(x.w.v1,j)),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
      x.pm = zdat.ash$PosteriorMean
      x.w = putD(x.w,j,x.pm)
    }
    mu.est=AvBasis(convert(x.w))
    ...
    x.w.v = wd.var(sigma^2, filter.number=1, family="DaubExPhase", type = "station")
    for(j in 0:(J-1)){
      zdat.ash=fast.ash(accessD(x.w,j),sqrt(accessC(x.w.v,j)),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
      x.pm = zdat.ash$PosteriorMean
      x.w = putD(x.w,j,x.pm)
...
bayestest.haar.un = function(x,sigma=NULL,prior="nullbiased",pointmass=TRUE,nullcheck=TRUE,gridmult=2,mixsd=NULL,VB=FALSE){
  n = length(x)
  J = log2(n)
  if(!isTRUE(all.equal(J,trunc(J)))){stop("Error: number of columns of x must be power of 2")}
  if(length(sigma)==1){sigma=rep(sigma,n)}
 ...
 if(is.null(sigma)){
    ...
    var.est2.ini=(rshift(x)-x)^2/2
      ...
      for(j in 0:(J-1)){
        ...
        zdat.ash=fast.ash(y[j+2,],sqrt(1*vtable1[j+2,]),prior=prior,pointmass=pointmass,nullcheck=nullcheck,VB=VB,mixsd=mixsd,gridmult=gridmult)
        wmean[j+1,] = zdat.ash$PosteriorMean/2
      }
      wwmean=-wmean
